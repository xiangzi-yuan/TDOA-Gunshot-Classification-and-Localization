...
% 初始位置和方向向量
initial_position = [0, 0, 0];  % Initial position in mm
movement_direction = [1, 0, 0];  % Movement direction vector, here stationary
...
zlabel('Z (mm)');
saxis = 500000;
axis([-saxis saxis -saxis saxis -saxis saxis]);
...
for k = 1:num_sensors
    ...
    if sensor_times_Machwave(k) ~= inf
        TDOA_machwave(k) = sensor_times_Machwave(k)-sensor_times_Machwave(1);
        fprintf('Sensor %d first encountered the mach wave at time %.6f seconds at position (%.0f, %.0f, %.0f).\n', ...
                k, sensor_times_Machwave(k), baseStation(k, 1), baseStation(k, 2), baseStation(k, 3));
    end
end
disp(TDOA);
disp(TDOA_machwave);